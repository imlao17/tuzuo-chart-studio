...
把知图的图表目录从当前 **30 个起始样式** 扩充到 **100 个**，覆盖 Flourish 生态中最主要、最常用的图表类型（以 Flourish 官方认可的 FT 可视化词汇表 + Flourish 模板目录为参照）。

知图的定位不变：**快速生成一张干净的、透明背景的、可直接复用的静态图表图片**。因此本计划只收录"能生成一张有意义的静态图"的图表；Flourish 中依赖滚动、动画播放、故事步骤的交互形态（如轮播、故事页、问答、体育阵容、音乐播放器）明确不在范围内。
...
- Flourish 的**动画竞赛类**（bar chart race / line chart race）→ 以静态终帧形态覆盖（`rankingBar`、`bump`），符合知图"生成图片"的定位。
