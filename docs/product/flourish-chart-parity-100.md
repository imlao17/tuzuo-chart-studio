...
|---|---|---|---|---|---|---|
| 1 | 条形/柱状扩展 | 10 | 已完成 | 2026-09-07 | 随本批提交（"Add 10 bar-family chart templates…"） | 密度直方图与象形柱状图两个 P2 均已完成；象形柱状图的「每单位代表值」暂不可配置（单位符号固定 10px 高 + 2px 间距），后续批次可补 |
| 2 | 折线/面积扩展 | 8 | 待开始 | — | — | |
...
| # | 中文名 | id | ECharts 方案 | 数据绑定要点 | 状态 |
|---|---|---|---|---|---|
| 11 | 点线图 | `pointLine` | line，symbol 大、线细 | 分类 + 多数值列 | 待做 |
| 12 | 平滑面积图 | `smoothArea` | line smooth + areaStyle | 分类 + 多数值列 | 待做 |
| 13 | 阶梯面积图 | `stepArea` | line step + areaStyle | 同上 | 待做 |
| 14 | 双轴折线图 | `dualAxisLine` | line×2 组，双 y 轴 | 分类 + ≥2 数值列（第 1 列左轴，其余右轴） | 待做 |
| 15 | 斜率图 | `slopeChart` | line，x 轴仅 2 个类目 | 分类列=系列名；恰好 2 数值列（两个时期） | 待做 |
| 16 | 区间面积图 | `bandArea` | 透明底线 + 面积 + 中值线 | 3 数值列：下限、上限、中值 | 待做 |
| 17 | 脊线图 | `ridgeline` | 多个 area 系列按行错位基线 | 分类（行/时期）+ 多数值列（每列一条脊） | 待做 |
| 18 | 凹凸图 | `bump` | line，renderer 内把值换成名次 | 分类（时期）+ 多数值列（每列一个主体，按列重算名次 1..n） | 待做 |
