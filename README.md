...
知图不是 Flourish 那样的发布/协作平台，而是专注一件事：**快速生成一张干净的、透明背景的、可直接复用的图表图片**。所有控件都真实影响预览与导出，每个模板都有独立的数据字段、校验、菜单和行为。当前编辑状态会自动保存在浏览器本地，也可以保存/打开 `.tuzuo.json` 项目文件。

## 模板与家族

100 个起始样式，按 renderer 家族组织，每个家族有独立的数据角色、设置菜单和能力声明：

| 家族 | 模板 |
|---|---|
| 柱/条 | 条形图、堆叠条形图、百分比条形图、柱状图、分组柱状图、堆叠柱状图、百分比柱状图、直方图、密度直方图、区间条形图、区间柱状图、子弹图、滑珠图、象形柱状图、进度条形图、排名条形图、帕累托图、分组条形图、胶囊条形图、箭头条形图、断轴条形图 |
| 折线/面积 | 折线图、平滑折线图、阶梯折线图、点线图、双轴折线图、斜率图、凹凸图、面积图、堆叠面积图、百分比面积图、平滑面积图、阶梯面积图、区间面积图、脊线图、河流图 |
| 饼/环 | 饼图、环形图、半环形图、多层环形图、玫瑰图、面积玫瑰图、径向条形图、径向堆叠条形图、进度环形图、极坐标折线图、极坐标面积图 |
| 组合 | 柱线组合图 |
| 散点/分布 | 散点图、气泡图、分组散点图、象限图、回归散点图、蜂群图、密度热力散点图 |
| 发散 | 发散条形图、人口金字塔 |
| 层级 | 旭日图、树状图、径向树图、组织架构图、矩形树图 |
| 网络 | 力导向网络图、弦图、邻接矩阵图、冲积图、桑基图 |
| 高级常用 | 点图、瀑布图、热力图、漏斗图、仪表盘、雷达图、箱线图、水平箱线图、蜡烛图 |
| 统计/分布 | 平行坐标图、日历热力图、累计分布图、误差线图、相关性矩阵图、小提琴图、马赛克图 |
| 卡片/文字 | 大数字卡、指标卡组、趋势迷你卡、条形表格图、词云 |
| 时间/分面 | 甘特图、时间线图、小倍数分面图 |
| 地图 | 世界分级统计地图、中国分级统计地图、符号地图、地点热力地图、流向地图 |
| 金融/比较 | 哑铃图、堆叠点图、OHLC 条形图、蜡烛+成交量组合图 |
...
| `EMAIL_FROM` | 验证邮件发件人，例如 `知图 <no-reply@example.com>` |
